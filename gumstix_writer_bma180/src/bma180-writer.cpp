...
#include <gumstix_writer_bma180/writermsg.h>
#include <errno.h>
...
int main(int argc, char** argv) {
	...
	// Ros
	ros::init(argc, argv, "gumstix_writer_bma180");
	ros::NodeHandle n;
	ros::Publisher pub = n.advertise<gumstix_writer_bma180::writermsg> ("writermsg", 10);	
	gumstix_writer_bma180::writermsg msg;
	...
	// get Parameters
	if (n.getParam("/gumstix_writer_bma180/rate_Hz", rate_Hz)==false) {
		rate_Hz = 1200;
	}
	if (n.getParam("/gumstix_writer_bma180/duration", duration)==false) {
		duration = 2;
	}
